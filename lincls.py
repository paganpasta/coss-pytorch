...
parser.add_argument("--subset", type=float, help='subset of images picked for finetuning', required=True)
...
def main():
    ...
    args.output = args.pretrained +f'.subset_{args.subset}.lincls'
    if not args.resume:
...
def main_worker(args, logger):
    ...
    if 'vit' in args.arch:
        model = models.__dict__[args.arch](num_classes=1000, is_eval=True)
    else:
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            print(msg)
            assert len(set(msg.missing_keys)) == 2, msg.missing_keys

            logger.info("=> loaded pre-trained model '{}'".format(args.pretrained))
        else:
            raise NotImplementedError(f'Pre-trained weight does not exist: {args.pretrained}')
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            try:
                ...
                print('Trained for', checkpoint['epoch'],' epochs with best val accuracy at', best_acc1)
                optimizer.load_state_dict(checkpoint['optimizer'])
                logger.info("=> loaded checkpoint '{}' (epoch {})".format(args.resume, checkpoint['epoch']))
            except:
                raise Exception('Not able to load args.resume')
            model.load_state_dict(checkpoint['state_dict'])
        else:
            logger.info("=> no resume checkpoint found at '{}'".format(args.resume))
    ...
    if args.evaluate:
        print(args.data)
        if 'imagenet-c' in args.data:
            val_loader = []
            import glob
            classes = sorted(glob.glob(args.data+'/*'))
            print(classes)
            for cls in classes:
                if not os.path.isdir(cls):
                    print(f'SKIPPING {cls}')
                    continue
                val_loader = torch.utils.data.DataLoader(ImageFolder(cls+'/1/', val_transform),batch_size=args.batch_size // dist.get_world_size(), shuffle=False, num_workers=args.workers, pin_memory=True)
                print(f'{cls}:')
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        logger.info(' * Acc@1 {top1.avg:.2f} Acc@5 {top5.avg:.2f}'
