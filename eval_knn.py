...
indices_in_1k = {v:k for k,v in thousand_k_to_200.items() if v != -1}
...
def get_dataloader(args, split):
    ...
    dataset_train = ReturnIndexDataset(os.path.join(args.data_path, split), transform=transform)
    ...
    return transform, dataset_train, data_loader_train


def extract_feature_pipeline(args, split='train'):
    transform, dataset_train, data_loader_train = get_dataloader(args, split)

    # ============ building network ... ============
    if 'timm' not in args.arch and args.arch in models.__dict__.keys():
    ...
    elif 'timm' in args.arch:
        model = timm.create_model(args.arch[len('timm.')], pretrained=True)
        model.fc = nn.Identity()
    else:
    ...
    train_features, train_indices = extract_features(model, data_loader_train)

    # save features and labels
    if args.dump_features:
        torch.save(train_features.cpu(), os.path.join(args.output, "trainfeat.pth"))
        torch.save(train_indices, os.path.join(args.output, "trainindices.pth"))
...
@torch.no_grad()
def knn_classifier(train_features, train_indices, k, num_classes=1000):
    train_features = train_features.t()
    num_test_images, num_chunks = train_indices.shape[0], 1000
    imgs_per_chunk = num_test_images // num_chunks
    outputs = []

    print(num_test_images, train_features.shape)

    for idx in range(0, num_test_images, imgs_per_chunk):
        # get the features for test images
        features = train_features[:, idx : min((idx + imgs_per_chunk), num_test_images)]
        # calculate the dot product and compute top-k neighbors
        similarity = torch.mm(features.t(), train_features)
        distances, indices = similarity.topk(k, largest=True, sorted=True)
        outputs.append(indices.cpu())
    return torch.cat(outputs, dim=0)


@torch.no_grad()
def knn_test_classifier(train_features, train_indices, test_features, k, num_classes=1000):
    train_features = train_features.t()
    test_features = test_features.t()
    num_test_images, num_chunks = test_features.shape[1], 1000
    imgs_per_chunk = num_test_images // num_chunks
    outputs = []
    
    print(num_test_images, train_features.shape, test_features.shape)
...
@torch.no_grad()
def classic_knn_classifier(train_features, train_labels, test_features, test_labels, k, T, num_classes=1000):
...
if __name__ == '__main__':
    ...
    if not args.dump_features and args.output and os.path.exists(os.path.join(args.output, 'trainfeat.pth')):
        ...
        train_features, train_indices = extract_feature_pipeline(args)
    
    _, train_db, _ = get_dataloader(args, 'train')
    
    if args.eval_path:
        args.data_path = args.eval_path
    
    test_features, test_indices = extract_feature_pipeline(args, split=args.split)
    _, test_db, _ = get_dataloader(args, args.split)

    print("Features are ready!\nStart the k-NN classification.")
    
    
    train_labels = torch.tensor([s[-1] for s in train_db.samples]).long()
    if 'imagenet-a' in args.data_path:
        print('modifying the labels')
        test_labels = torch.tensor([indices_in_1k[s[-1]] for s in test_db.samples]).long()
    else:
        test_labels = torch.tensor([s[-1] for s in test_db.samples]).long()
    ...
    #    #top51_indices = knn_classifier(train_features, train_indices, 51) 
    #    #torch.save(top51_indices, os.path.join(args.dump_features, '.output'))
    #    torch.save(top51_test_indices, os.path.join(args.output, 'output.test'))
